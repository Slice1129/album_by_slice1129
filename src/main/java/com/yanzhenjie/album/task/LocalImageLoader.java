...
public class LocalImageLoader implements AlbumImageLoader {
    ...
    private static ArrayList<String> lurCacheKey = new ArrayList<String>();
    ...
    private void addImageToCache(String key, int width, int height, Bitmap bitmap) {
        if (getImageFromCache(key, width, height) == null && bitmap != null)
            synchronized (mLruCache) {
                ...
                lurCacheKey.add(key);
                Log.e("FUCK", "添加成功" + key + "，当前大小：" + lurCacheKey.size());
            }
    }

    public static void clearLruCache() {
        if (lurCacheKey != null && mLruCache != null) {
            Log.e("FUCK", "lurCacheKey数量：" + lurCacheKey.size());
            Log.e("FUCK", "putCount" + mLruCache.putCount());
            for (String key : lurCacheKey) {
